...
<script setup>
import { ref, watchEffect, onMounted, nextTick, watch, onUnmounted } from "vue";
...
let contentChunks = []; // 拆分后的内容片段（字符数组）
let streamTimer = null; // 流式输出定时器
// 确保正确处理Markdown内容
...
`;
const startStreamRender = fullContent => {
  // 清除旧定时器（避免重复触发）
  if (streamTimer) clearInterval(streamTimer);
  //重置状态
  streamContent.value = "";
  parsedContent.value = "";
  // 拆分完整内容为字符数组（按字符拆分，效果更细腻）
  contentChunks = fullContent.split("");
  let currentIndex = 0; // 当前追加到的索引
  ...
  streamTimer = setInterval(() => {
    ...
    // 追加当前字符
    streamContent.value += contentChunks[currentIndex];
    currentIndex++;
    // 解析并更新 DOM
    parsedContent.value = parseMarkdown(streamContent.value);
    // 重新初始化剪贴板（新代码块需重新绑定）
...
watch(
  ...
  newFullContent => {
    if (newFullContent) {
      startStreamRender(newFullContent);
    }
  },
  { immediate: true } // 初始传入内容时立即触发
);
// 动态注入样式
onMounted(() => {
